...
| --- | --- | --- | --- | --- | --- |
| `rsrcd/` | rsrcd | StackImport-owned support library for typed Classic Mac resource fork parsing, derived from rsrcdump | MIT, see `rsrcd/LICENSE` | `vendor_rsrcd` interface target for now; treated as core StackImport infrastructure and used directly by importer internals | No upstream source changes. Added `rsrcd/STACKIMPORT_VENDOR.md` and CMake integration in `vendor/CMakeLists.txt`; future work should move it closer to core typed resource APIs rather than hiding it behind pointer/size shims. |
| `quill/` | Quill 11.1.0 | Header-only C++ logging backend used by stackimport diagnostics | MIT, see `quill/LICENSE` | `vendor_quill` interface target; used privately by core stackimport targets; stackimport's logging facade uses Quill console sinks | No upstream source changes. Added `quill/STACKIMPORT_VENDOR.md` and CMake wrapper in `vendor/CMakeLists.txt`. |
| `rang/` | rang 3.2 | Header-only terminal color support for log/dump formatting | Unlicense, see `rang/LICENSE` | `vendor_rang` interface target; used privately by core stackimport targets for terminal color helpers | No upstream source changes. Added `rang/STACKIMPORT_VENDOR.md` and CMake wrapper in `vendor/CMakeLists.txt`. |
| `rapidjson/` | RapidJSON 1.1.0 | Header-only JSON DOM/writer used by core stackimport targets | MIT, see `rapidjson/license.txt` | `vendor_rapidjson` interface target; used privately by core stackimport targets | No upstream source changes. Added `rapidjson/STACKIMPORT_VENDOR.md` and CMake wrapper in `vendor/CMakeLists.txt`. |
| `cli11/` | CLI11 2.6.2 | Header-only command-line parser used by the `stackimport` executable | BSD-3-Clause, see `cli11/LICENSE` | `vendor_cli11` interface target; used privately by the `stackimport` executable only | No upstream source changes. Added `cli11/STACKIMPORT_VENDOR.md` and CMake wrapper in `vendor/CMakeLists.txt`. |
| `resource_dasm/` | `resource_dasm` / `libresource_file` snapshot | Classic Mac resource fork and resource conversion reference implementation | MIT, see `resource_dasm/LICENSE` | Built by `vendor_resource_dasm` external project; included in `stackimport-vendor-static`; MACE and code-resource disassembly support are exposed to core StackImport through local resource_dasm adapter sources | Local CMake patch makes warnings-as-errors configurable, disables C++ extensions, removes hardcoded `-O2`, declares `DISABLE_SDL`, links audio tools through `resource_file`, and preserves the static `resource_file` library build. Local integration forwards C++23/C17 and strict flags, including `-Wpedantic`/`-Wformat=2` with a Clang zero-length-array compatibility suppression. |
| `phosg/` | `phosg` snapshot | Required support library for `resource_dasm`; SHA-256 helper for ROM analysis | MIT, see `phosg/src/LICENSE` | Built by `vendor_phosg` external project before `vendor_resource_dasm`; hashing is exposed through the local `StackImportPhosgHashAdapter` source | Local CMake patch disables C++ extensions and makes warnings-as-errors configurable so the wrapper controls the same standard and warning policy as the rest of the build. Local integration forwards C++23/C17 and strict flags, including `-Wpedantic`/`-Wformat=2`. |
...
| `stackimport-vendor-static` | Top-level target that combines the internal `libstackimport_core.a` with vendored static archives into the public `libstackimport.a`. |
