...
| --- | --- | --- | --- | --- | --- |
| `rsrcd/` | rsrcd | Header-only C++ resource fork parser derived from rsrcdump | MIT, see `rsrcd/LICENSE` | `vendor_rsrcd` interface target; used privately by core stackimport targets; ICON/CURS/PAT# image decoding is wrapped by `StackImportRsrcdAdapter` | No upstream source changes. Added `rsrcd/STACKIMPORT_VENDOR.md` and CMake integration in `vendor/CMakeLists.txt`. |
