...
description: Parser correctness, C API behavior, and vendor-boundary remediation plan.
---
...
- `clut`, `CTBL`, `actb`, `cctb`, `dctb`, `fctb`, and `wctb` color tables now
  have an rsrcd typed parser and shared transform for seed, flags, 16-bit RGB
  entries, and PNG swatch previews.
...
- `ppat` and `ppt#` pixel patterns now have rsrcd typed metadata parsers and
  ...
  pixmap metadata where present. ROM short-form `ppat` resources that use a
  compact PixMap-shaped payload now fall back to compact indexed PixMap PNG
  output.
- `pixs` ROM resources with compact indexed PixMap headers now decode to PNG
  through the shared transform layer. Larger non-compact `pixs` payloads remain
  raw until their layout is understood.
...
- Current typed StackImport/core coverage:
  ...
  `cfrg`, `cctb`, `dctb`, `fctb`, `wctb`, `pltt`, `pixs`, `ppat`, `ppt#`, `HCbg`, `HCcd`, `STR `,
